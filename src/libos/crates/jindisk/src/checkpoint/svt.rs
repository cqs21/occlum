...
pub struct SVT {
    ...
    bitmap: BitMap,
    disk_array: DiskArray<u8>,
}

const SVT_ENCRYPTED_WITH_HMAC: bool = true;
...
impl SVT {
    pub fn new(
        ...
        Self {
            ...
            disk_array: DiskArray::new(
                (num_segments + BITMAP_UNIT - 1) / BITMAP_UNIT,
                DiskShadow::new(svt_boundary, disk),
                key,
                SVT_ENCRYPTED_WITH_HMAC,
            ),
        }
    }
    ...
    pub fn calc_svt_blocks(num_segments: usize) -> usize {
        let nr_units = (num_segments + BITMAP_UNIT - 1) / BITMAP_UNIT;
        DiskArray::<u8>::total_blocks(nr_units)
    }

    /// Calculate space cost in bytes (with shadow blocks) on disk.
    pub fn calc_size_on_disk(num_segments: usize) -> usize {
        let nr_units = (num_segments + BITMAP_UNIT - 1) / BITMAP_UNIT;
        let total_blocks = DiskArray::<u8>::total_blocks_with_shadow(nr_units);
...
impl SVT {
    pub async fn persist(&mut self, checkpoint: bool) -> Result<bool> {
        let bitmap_raw_slice = self.bitmap.as_raw_slice();
        for offset in 0..bitmap_raw_slice.len() {
            let unit = bitmap_raw_slice[offset];
            self.disk_array.set(offset, unit).await?;
        }
    ...
    pub async fn load(
        ...
        let mut disk_array = DiskArray::new(nr_units, disk_shadow, key, SVT_ENCRYPTED_WITH_HMAC);

        let mut bitmap = BitMap::with_capacity(num_segments);
        for offset in 0..nr_units {
            let value = disk_array.get(offset).await?;
...
mod tests {
    ...
    fn test_svt_persist_load() -> Result<()> {
        async_rt::task::block_on(async move {
            ...
            let svt_blocks = SVT::calc_svt_blocks(num_segments);
            let boundary = HbaRange::new(svt_addr..svt_addr + (svt_blocks as _));
            let key = DefaultCryptor::gen_random_key();
            ...
            let shadow = svt.persist(true).await?;
