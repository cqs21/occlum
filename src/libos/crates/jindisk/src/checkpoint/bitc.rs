...
const BITC_ENCRYPTED_WITH_HMAC: bool = true;

impl BITC {
    pub fn new(
        index_region_addr: Hba,
        num_index_segments: usize,
        ...
        Self {
            ...
            disk_array: DiskArray::new(
                num_index_segments,
                DiskShadow::new(bitc_boundary, disk),
                key,
                BITC_ENCRYPTED_WITH_HMAC,
            ),
        }
    }
...
impl BITC {
    ...
    pub async fn load(
        ...
        let mut disk_array = DiskArray::new(
            index_svt.num_segments(),
            disk_shadow,
            key,
            BITC_ENCRYPTED_WITH_HMAC,
        );
        ...
        for offset in index_svt.bitmap().iter_zeros() {
            let bit: DiskBit = disk_array.get(offset).await?;
            if max_bit_version < *bit.version() {
                max_bit_version = *bit.version();
            }
            match *bit.level() {
                0 => l0_bit = Some(Bit::new(bit)),
                1 => {
...
mod tests {
    ...
    fn test_bitc_persist_load() -> Result<()> {
        async_rt::task::block_on(async move {
            ...
            let bit_addr = fake_svt.pick_avail_seg().unwrap();
            ...
            let shadow = bitc.persist(true).await?;
