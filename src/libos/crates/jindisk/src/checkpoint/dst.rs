...
const DST_ENCRYPTED_WITH_HMAC: bool = true;
...
impl DST {
    pub fn new(
        ...
        let nr_units = (nr_bits + BITMAP_UNIT - 1) / BITMAP_UNIT;
        Self {
...
impl DST {
    pub async fn persist(&mut self, checkpoint: bool) -> Result<bool> {
        ...
        for (seg_addr, bitmap) in bitmaps.iter() {
            for i in 0..bitmap.as_raw_slice().len() {
                ...
                let unit = bitmap.as_raw_slice()[i];
    ...
    pub async fn load(
        ...
        let mut disk_array = DiskArray::new(nr_units, disk_shadow, key, DST_ENCRYPTED_WITH_HMAC);
...
mod tests {
    ...
    fn test_dst_fns() {
        ...
        let mut dst = DST::new(region_addr, num_segments, dst_boundary, disk, key);
    ...
    fn test_dst_persist_load() -> Result<()> {
        async_rt::task::block_on(async move {
            ...
            let shadow = dst.persist(true).await?;
